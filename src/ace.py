...
class ConceptDiscovery(object):
  ...
  def __init__(self,
    ...
    For a trained ResNet_pytorch model, the ConceptDiscovery class first
    performs unsupervised concept discovery using examples of one of the classes
    in the network.

    Args:
      model: A trained ResNet_pytorch model on which we run the concept
             discovery algorithm
  ...
  def create_patches(self, method='slic', discovery_images=None,
                     param_dict=None, gradcam=False, keep_percent=80):
    ...
    if discovery_images is None:
      raw_imgs = self.load_concept_imgs(
          self.target_class, self.num_discovery_imgs, compute_tcav=True)
    ...
    if self.num_workers:
      ...
      if gradcam:
        outputs = pool.map(
            lambda img: self._return_gradcam_superpixels(img, method, param_dict, keep_percent),
  ...
  def _return_gradcam_superpixels(self, img, method='slic',
                          param_dict=None, keep_percent=80):
  ...
  def cavs(self, min_acc=0., ow=True):
    ...
    Args:
      min_acc: Delete discovered concept if the average ResNet_pytorch accuracy
        of the CAV is less than min_acc
      ow: If True, overwrites an already calcualted cav.

    Returns:
      A dictionary of ResNet_pytorch accuracy of linear boundaries orthogonal
      to cav vectors
    """
    ...
    for bn in self.bottlenecks:
      ...
      for i, image in enumerate(images):
        bn_grads[i] = self.model.get_gradient(
            None, [class_id], bn, image).reshape(-1)
